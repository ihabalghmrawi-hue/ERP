...
export const SaaSDB = {
  ...
  // ── Session Management ─────────────────────────────────
  setSession(data: { type: "superadmin" | "company"; id: string; name: string; email: string }): void {
    if (typeof window === "undefined") return;
    localStorage.setItem(SESSION_KEY, JSON.stringify(data));
  },

  getSession(): { type: "superadmin" | "company"; id: string; name: string; email: string } | null {
